...
9. No Error Leaks: PASS — All client-visible API responses return generic error strings; Momence routes gate `details:` on `NODE_ENV === 'development'` only; `sendToFacebookCAPI` error details logged server-side only, never returned to clients; no stack traces or raw error objects in production responses
