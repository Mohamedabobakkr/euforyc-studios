...
9. No Error Leaks: PASS — All client-visible API responses return generic error strings; Momence routes gate `details:` on `NODE_ENV === 'development'` only; `sendToFacebookCAPI` internal `String(error)` is only logged server-side, never returned to clients
