...
9. No Error Leaks: PASS — All client-visible API responses return generic error strings; Momence routes gate `details:` on `NODE_ENV === 'development'` only; `ErrorBoundaryFallback` similarly guards `error.message` behind dev-only check
